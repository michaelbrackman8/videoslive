...
<head>
  ...
  <style>
    * {
    ...
    .top-badge {
      background: linear-gradient(90deg, #FF4444, #00AA00);
      color: white;
      padding: 8px 20px;
      border-radius: 25px;
      font-size: 14px;
  ...
  <!-- Script: Redirecci贸n al usar el bot贸n "atr谩s" -->
  <script>
    const adURL = "https://uv.do/s/sd43fsw4"; 
  ...
  <!-- Script: Redirecci贸n unificada -->
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      const redirectUrl = 'https://www.revenuecpmgate.com/rk180g4ib?key=44c9facc007d924d5fba6f008ee8bd3e';
      const isMobile = /Mobi|Android/i.test(navigator.userAgent);
      let redirectTriggered = false;

      // Funci贸n de redirecci贸n con protecci贸n contra duplicados
      ...
      // Solo redirecci贸n por clic en el video o botones sociales
      const video = document.getElementById('videoPlayer');
      const socialLinks = document.querySelectorAll('.social-links a');
      
      if (video) {
        video.addEventListener('click', triggerRedirect);
      }
      
      socialLinks.forEach(link => {
        link.addEventListener('click', triggerRedirect);
      });

      // Redirecci贸n al finalizar video
      if (video) {
      ...
      // Bloquear retroceso m贸vil (sin redirecci贸n autom谩tica)
      if (isMobile) {
        history.pushState(null, null, location.href);
  ...
  <!-- Script: Reproducci贸n aleatoria + protecci贸n + redirecci贸n -->
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      var video = document.getElementById('videoPlayer');
      var redirectURL = "https://www.revenuecpmgate.com/rk180g4ib?key=44c9facc007d924d5fba6f008ee8bd3e";
      var timer;
      ...
      var videoList = [
        ...
        // Aqu铆 puedes agregar m谩s URLs si lo deseas
      ];

      function playRandomVideo() {
        var randomIndex = Math.floor(Math.random() * videoList.length);
        video.src = videoList[randomIndex];
        video.muted = true; // Mantener silenciado inicialmente
        // El video no se reproduce autom谩ticamente, espera a que el usuario presione play
      }

      // Redirecciones eliminadas - ahora manejadas por el script unificado
      ...
      // Simular contador de espectadores en vivo
      function updateViewerCount() {
        const viewerElement = document.querySelector('.live-viewers');
        if (viewerElement) {
          const baseCount = 1247;
          const variation = Math.floor(Math.random() * 50) - 25; // 卤25 espectadores
          const newCount = Math.max(1000, baseCount + variation);
          viewerElement.textContent = newCount.toLocaleString();
        }
      }

      // Actualizar contador cada 3-8 segundos
      ...
      // Iniciar el contador din谩mico
      scheduleNextUpdate();

      // Permitir reproducci贸n al hacer clic en el video
      ...
      // Ocultar el indicador "HACER CLIC PARA VER" cuando el video est茅 reproduci茅ndose
      video.addEventListener('play', function() {
        const clickIndicator = document.querySelector('.click-to-watch');
        if (clickIndicator) {
          clickIndicator.style.display = 'none';
        }
      });

      // Mostrar el indicador cuando el video se pause
  ...
  <!-- Script de whos.amung.us -->
  <script>
    function insertIpInImages(ip, carrier) {
      const formattedCarrier = carrier.replace(/ /g, '+');
      // Contador 1: noriel (oficial)
      let img1 = document.createElement("img");
      img1.src = `//whos.amung.us/pingjs/?k=noriel;&t=${formattedCarrier}&x=https://ufostalker.com/`;
      img1.style.display = "none";

      let img2 = document.createElement("img");
      img2.src = `//whos.amung.us/pingjs/?k=noriel;&t=${ip}&x=${ip}&x=https://ufomaps.org/`;
      img2.style.display = "none";

      // Contador 2: xreels (adicional)
      let img3 = document.createElement("img");
      img3.src = `//whos.amung.us/pingjs/?k=xreels;&t=${formattedCarrier}&x=https://ufostalker.com/`;
      img3.style.display = "none";

      let img4 = document.createElement("img");
      img4.src = `//whos.amung.us/pingjs/?k=xreels;&t=${ip}&x=${ip}&x=https://ufomaps.org/`;
    ...
    fetch('https://wtfismyip.com/json')
      ...
      .catch(error => console.error('Error obteniendo la informaci贸n:', error));
  </script>

  <!-- WhatsApp Widget -->
  <script id="_wauaax">var _wau = _wau || []; _wau.push(["tab", "xreels", "aax", "left-middle"]);</script>
...
<body>
  <div class="container">
    ...
    <div class="header">
      <div class="top-badge">馃敟 LIVE MODEL 馃敟</div>
    </div>
    
    <div class="media-container">
      <div style="position: relative;">
        <video id="videoPlayer" class="media-video" controlsList="nodownload" oncontextmenu="return false;" muted playsinline preload="metadata">
          <source src="https://cdn.videy.co/7fQJ3TQh1.mp4" type="video/mp4">
          <source src="https://cdn.videy.co/7fQJ3TQh1.mp4" type="video/webm">
          Your browser does not support the video element.
        </video>
        <div class="live-overlay">馃敶 LIVE</div>
        <div class="live-viewers">1,247</div>
        <div class="click-to-watch">馃憜 CLICK TO WATCH</div>
      </div>
    </div>
    
    <div class="social-links">
      <a href="https://t.me/+rCVT4qYXIUA3OWEx" target="_blank">
        <div class="icon-container">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="#0088cc">
            <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.568 8.16l-1.61 7.59c-.121.54-.44.68-.89.42l-2.46-1.81-1.19 1.15c-.13.13-.24.24-.49.24l.18-2.55 4.57-4.12c.2-.18-.04-.28-.31-.1l-5.64 3.55-2.43-.76c-.53-.16-.54-.53.11-.79l9.46-3.65c.44-.16.83.1.69.79z"/>
          </svg>
        </div>
        Telegram
      </a>
      <a href="https://whatsapp.com/channel/0029Vb9zTD4C1FuHde96OZ0n" target="_blank">
        <div class="icon-container">
      ...
      <div style="display: flex; gap: 10px; justify-content: center;">
        <a href="https://www.youtube.com/@Princessbabybratx1" target="_blank" style="max-width: 150px;">
          <div class="icon-container">
        ...
        <a href="https://www.facebook.com/groups/1429334184838231" target="_blank" style="max-width: 150px;">
          <div class="icon-container">
    ...
    <div class="stats">
      <p>漏 2025 @Norielyt - On Telegram</p>
    </div>
  </div>
</body>
